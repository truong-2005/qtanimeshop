...
const CategoryList = () => {
  ...
  const renderRow = (category) => (
    <tr key={category.id} className="hover:bg-slate-900/40 border-b border-slate-900 transition-colors">
      <td className="px-6 py-4 font-semibold text-slate-400">#{category.id}</td>
      <td className="px-6 py-4 font-bold text-slate-200">{category.name}</td>
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex justify-between items-center">
        <Title subtitle="Quản lý các nhóm sản phẩm figure">
          Danh mục ({filtered.length})
        </Title>
      ...
      <div className="flex justify-start">
        <SearchBox placeholder="Tìm kiếm danh mục..." onSearch={setSearchQuery} />
      </div>
